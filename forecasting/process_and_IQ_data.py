...
def format_forecast_pivot_no_date(
    ...
    for col in sorted_month_cols:
        wide_df[col] = wide_df[col].apply(lambda x: f"{x:.2f}" if pd.notnull(x) else "")
    return wide_df
        ...
        for i in range(n_rows):
            ...
            if pd.isna(val) or val == 0:
                if i == n_rows - 1:
                    prev_row = i - 1
                    prev2_row = i - 2
                    prev_month = month_columns[col_idx] if col_idx >= 0 else None
                    prev_row_curr_month = (
                    ...
                    prev_row_prev_month = (
                        df_copy.iloc[prev_row][prev_month]
                        if prev_row >= 0 and prev_month
                        else None
                    )
                    prev2_row_prev_month = (
                        df_copy.iloc[prev2_row][prev_month]
                        if prev2_row >= 0 and prev_month
                        else None
                    )
                    curr_row_prev_month = (
                        df_copy.iloc[i][prev_month] if prev_month else np.nan
                    )
                    curr_row_month = df_copy.iloc[i][curr_month]

                    if prev_row >= 0 and prev2_row >= 0:
                        if pd.isna(prev2_row_curr_month) or prev2_row_curr_month == 0:
                            if prev_month and not pd.isna(prev_row_prev_month) and prev_row_prev_month != 0:
                            ...
                            if ratio2 < 0.8 or ratio2 > 1.2:
                                ...
                                imput_val = (
                                    np.mean(prev_3_month_values)
                                    if prev_3_month_values
                                    else np.nan
                                )
                            else:
                                imput_val = (
                                    ratio2 * curr_row_prev_month
                                    if not pd.isna(curr_row_prev_month)
                                    and curr_row_prev_month != 0
                                    else np.nan
                                )
                        else:
                            avg_num = np.mean([curr_row_month, prev_row_prev_month])
                            avg_den = np.mean([prev_row_prev_month, prev2_row_curr_month])
                            if not pd.isna(prev_row_curr_month) and prev_row_prev_month != 0:
                                avg_num = np.mean([prev_row_curr_month, prev_row_prev_month])
                                avg_den = np.mean([prev_row_prev_month, prev2_row_curr_month])
                            if avg_den != 0 and not pd.isna(avg_den):
...
def plot_contact_ratio_seasonality(
    ...
    imputed_df = impute_blanks_with_directional_average(df, months)
    seasonality = calculate_seasonality(imputed_df)
    ...
    alignments = ["bottom left", "top right", "top left", "top right"]
    offsets = [-10, 0, 10]
...
def prepare_styled_forecast_for_model(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in numeric_cols:
        df[col] = df[col].apply(lambda x: f"{x:.2f}x" if pd.notnull(x) else "")
    return df
...
def accuracy_phase1(
    ...
    try:
        ...
        def filter_and_summarize(row: pd.Series, lower: float, upper: float):
            ...
            total = len(filtered)
            accuracy = count / total * 100 if total > 0 else None
        ...
        columns_to_format = ["Accuracy(+−5%)", "Accuracy(+−7%)", "Accuracy(+−10%)"]
